...
class TypedLine:
    ...
    """
    def __init__(self, line: QLine, type_line: str, atom_indexes: Optional[List[int]] = None):
        self.line = line
        self.type = type_line
    ...
    def draw(self, painter: QPainter, constants: Dict) -> None:
        ...
        pen = QPen(PASTEL_RED, constants["line_width"])
        painter.setPen(pen)
        ...
        if self.type == "single":
            ...
            if px: 
                painter.drawLine(QLine(self.line.x1() + constants["bond_distance"] * px, 
            ...
            if px:
                painter.drawLine(QLine(self.line.x1() - constants["bond_distance"] * px, 
                                       ...
                                       self.line.x2() - constants["bond_distance"] * px, 
                                       self.line.y2() - constants["bond_distance"] * py))
                dotted_line = QLine(self.line.x1() + constants["bond_distance"] * px, 
                                    self.line.y1() + constants["bond_distance"] * py, 
                                    self.line.x2() + constants["bond_distance"] * px, 
                                    self.line.y2() + constants["bond_distance"] * py)
                
                dotted_pen = QPen(PASTEL_RED, constants["line_width"], Qt.DotLine)
                painter.setPen(dotted_pen)
                painter.drawLine(dotted_line)
        
        elif self.type == "solid unwedge":
            pen = QPen(PASTEL_RED, int(constants["line_width"] * 1.5))
            painter.setPen(pen)
            painter.drawLine(self.line)
            ...
            pen = QPen(PASTEL_RED, int(constants["line_width"] * 0.8))
            painter.setPen(pen)
